...
public class Player : MonoBehaviour
{
    ...
    // knockback mais forte quando a fonte do dano � conhecida
    public Vector2 strongDamageKnockback = new Vector2(6f, 8f);
    // dura��o (segundos) que o jogador fica sem controle horizontal ap�s knockback
    ...
    // novo: multiplicador para pulo vertical reduzido quando saltando "grudado" ou em dire��o � parede
    public float smallWallJumpMultiplier = 0.7f;

    // novo: tempo que o jogador fica "grudado" antes de come�ar a deslizar
    ...
    void Update()
    {
        // DEBUG: pressione L para listar colliders pr�ximos (Overlap) e C para listar contatos atuais do Rigidbody
        ...
        // Double jump (executado ap�s WallJump para n�o conflitar com wall-jump)
        ...
        // permite flip mais cedo: agora FlipIfNeeded � aplicado normalmente (controle horizontal pode tomar efeito cedo)
        if (!isWallJumping)
        {
            FlipIfNeeded();
        }
        ...
        {
            // mesmo durante wall-jump permita alterarmos facing se o jogador j� estiver pressionando dire��o oposta
    ...
    private void FixedUpdate()
    {
        ...
        // Permite ao jogador modificar a dire��o horizontal mais cedo, mesmo durante um wall-jump.
    ...
    private bool IsWalled()
    {
        ...
        foreach (var col in cols)
        {
            ...
            // ignora triggers (n�o devem bloquear movimento)
            ...
            // ignora colisor do pr�prio jogador (por seguran�a)
            if (col.gameObject == gameObject) continue;

            // log para identificar o que est� bloqueando
    ...
    private void WallSlide()
    {
        ...
        // se o jogador pressionar dire��o oposta � parede enquanto grudado -> cancela o "stick" e come�a a deslizar imediatamente
        ...
        if (touchingWall)
        {
            ...
            // se o jogador estiver pressionando na dire��o contr�ria � parede, quebra o "stick"
            ...
            // quando o timer expira, come�a o sliding normal (limita velocidade de queda)
            isWallSliding = true;
        ...
        else
        {
            wasTouchingWall = false;
            wallStickTimer = 0f;
            // padr�o: se estiver caindo e n�o em parede, aplicar clamp normal (j� feito por outras rotinas), aqui apenas zera isWallSliding
            isWallSliding = false;
        }
    }
    ...
    private void WallJump()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Space) && wallJumpingCounter > 0f)
        {
            ...
            // dire��o aproximada da parede (se existir)
            float wallDir = 0f;
            if (wallCheck != null) wallDir = Mathf.Sign(wallCheck.position.x - transform.position.x);
            ...
            {
                // Caso B: se n�o houver input horizontal -> salto diagonal na dire��o contr�ria � que est� olhando
                if (!hasDirectionalInput)
                {
                    float dir = -Mathf.Sign(transform.localScale.x); // oposto ao que est� olhando
                    rb.linearVelocity = new Vector2(dir * wallJumpingPower.x, wallJumpingPower.y);
                    // N�O invertemos o facing automaticamente: jogador continua olhando para onde estava
                }
                else
                {
                    // Caso C: input horizontal presente -> normal wall-jump na dire��o do input
                    float dir = Mathf.Sign(inputHor);
                    rb.linearVelocity = new Vector2(dir * wallJumpingPower.x, wallJumpingPower.y);

                    // ajusta facing se necess�rio
            ...
            // Permite altera��o de trajet�ria cedo (FixedUpdate j� aplica horizontal conforme input).
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        {
            // passa a posi��o do colisor que causou o dano para aplicar knockback direcional
    ...
    public void TakeDamage(int damage, Vector2? sourcePosition = null)
    {
        ...
        // c�lculo de knockback direcional:
        float horizKnock = rb.linearVelocity.x; // fallback mant�m velocidade atual no X
        ...
        if (sourcePosition.HasValue)
        {
            ...
            // horizontal forte sempre na dire��o contr�ria � que o jogador est� olhando
            ...
            else
            {
                ...
                if (delta.y < 0f)
                {
                    // fonte abaixo -> lan�a para cima tamb�m
                    vertKnock = strongDamageKnockback.y;
                }
                else
                {
                    // fonte acima -> n�o lan�a para baixo, apenas horizontal
                    vertKnock = 0f;
                }
            }
        }
        ...
        {
            // sem posi��o do atacante: comportamento antigo (lan�a pra cima)
